...
//fiter menu
let fiterBtn = document.querySelector('.sortby-filter__btn');
let fiterMenu = document.querySelector('.catalog__filter');
let filterTitle = document.querySelector('.filter-title');
let filterOverlay = document.querySelector('.filter-overlay');

function openFilter() {
  fiterMenu.classList.toggle('active');
  filterTitle.style.borderRadius = 'unset';
  body.classList.toggle('disable');
  filterOverlay.classList.toggle('open');
}

function closeFilterMenu() {
  fiterMenu.classList.remove('active');
...
function openTab(evt, cityName) {
  ...
  for (i = 0; i < x.length; i++) {
    tablinks[i].className = tablinks[i].className.replace(" tab-bg-active", "");
  }
  document.getElementById(cityName).style.display = "block";
  evt.currentTarget.className += " tab-bg-active";
}